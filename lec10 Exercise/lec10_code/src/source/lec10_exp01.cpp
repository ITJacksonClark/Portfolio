// A filled-in rectangle that is blue but half of it
// changes color to magenta or dark violet when
// pressing certain keys while running it 
// By Dr. Qi Zhang
// 2/12/24
...
void display()
{
	...
	//When we draw the triangle, we first use the vertex and fragment 
	...
	//the second VAO.
	//When we draw the second triangle, we want to use a different 
	//shader program, so we switch to the second shader program.
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Draw Two Triangles Using Shaders", NULL, NULL);
	if (window == NULL)
	{
