// A cube that starts cyan but can turn
// emerald by pressing E and then can
// turn back to cyan by pressing C
...
void init(void)
{
	...
	zNumDif = 0.50980392f;
	yNumDif = 0.50980392f;
	xNumSpec = 0.50196078f;
	zNumSpec = 0.50196078f;
	yNumSpec = 0.50196078f;
...
void display()
{
	...
	glUniform3f(materialAmbientLoc, xNumAmb, yNumAmb, zNumAmb);

	unsigned int materialDiffuseLoc = glGetUniformLocation(lightingShader, "material.diffuse");
	glUniform3f(materialDiffuseLoc, xNumDif, yNumDif, zNumDif);
	...
	glUniform3f(materialSpecularLoc, xNumSpec, yNumSpec, zNumSpec);
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Example 2n: Materials", NULL, NULL);
	if (window == NULL)
	{
